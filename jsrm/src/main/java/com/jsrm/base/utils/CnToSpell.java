		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			...
			if (ascii == 0) { // 取ascii时出错
				...
				if (spell == null) {
					...
					if(!"".equalsIgnoreCase(spell)){
						spell=spell.substring(0,1);
					}
					retuBuf.append(spell.toLowerCase());
...
//		} catch (Exception ex) {
//		}
//		System.out.println("全拼："+CnToSpell.getFullSpell(str));
//		System.out.println("简拼："+CnToSpell.getSimpleSpell(str));
//	}
}