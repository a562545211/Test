 ...
 * 根据学段、学科初始化知识点
 * */
function initKnowledgesBy2(id_num,phase,subject) {
    var phaseId,subjectId;
    if(phase !== undefined ){
        phaseId = phase
    }else{
        phaseId = $("#selectbox1 .selectbox_selected").attr("data-id");
    }
    if(subject !== undefined ){
        subjectId = subject
    }else{
        subjectId = $("#selectbox2 .selectbox_selected").attr("data-id");
    }
    ...
    function firstBtnAdapter(data) {
        ...
        if (data.length != 0) {
            ...
            for (var i = _num; i <= FIXNUMBER; i++) {
                ...
                if (_num == 3) _k = "三";
                if (_num == 4) _k = "四";
                $("#selectboxKnowledge" + _num).selectBox("removeAll");
                $("#selectboxKnowledge" + _num).css("background-color", "#eaecf2");
